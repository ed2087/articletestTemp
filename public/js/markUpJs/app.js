                    ...
                    if (response.ok) {
                        ...
                        replacementText = `
                            &#8204;<div class="img_content_wrap">
                                <img src="${imageData.url}" alt="${imageFile.name}" style="${style}" data-public-id="${imageData.public_id}">                            
                            </div>&#8204;
                        `;
                        updateContentContainer(replacementText);
                    } else {
        ...
        for (const mutation of mutationsList) {
            if (mutation.removedNodes.length > 0) {
                mutation.removedNodes.forEach(node => {
                    if (node.nodeType === 1 && node.tagName === 'IMG' && node.dataset.publicId) {
                        console.log('Image removed:', node);
                        removedImages.push(node.dataset.publicId);
                    }
                });
            }
        }
    ...
    const deleteImages = async (publicIds) => {
        ...
        try {
            const response = await fetch('/delete/images', {
                ...
                body: JSON.stringify({ public_ids: publicIds }) // Send array of public_ids in the request body
            });
...
//                     if (node.nodeType === 1 && node.tagName === 'IMG' && node.dataset.publicId) {
//                         console.log('Image removed:', node);
//                         const publicId = node.dataset.publicId;
...
//         try {
//             const response = await fetch(`/delete/images/${publicId}`, { // Ensure publicId is directly appended
//                 method: 'DELETE',
//                 headers: {
//                     'X-CSRF-Token': csrfToken,
//                     'Content-Type': 'application/json'
//                 },
//                 body: JSON.stringify({ public_id: publicId }) // Send public_id in the request body
//             });
